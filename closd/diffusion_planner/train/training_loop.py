...
from closd.diffusion_planner.diffusion import logger
from closd.diffusion_planner.utils import dist_util
from closd.diffusion_planner.diffusion.fp16_util import MixedPrecisionTrainer
from closd.diffusion_planner.diffusion.resample import LossAwareSampler, UniformSampler
from tqdm import tqdm
from closd.diffusion_planner.diffusion.resample import create_named_schedule_sampler
from closd.diffusion_planner.data_loaders.humanml.networks.evaluator_wrapper import EvaluatorMDMWrapper
from closd.diffusion_planner.eval import eval_humanml
from closd.diffusion_planner.sample.generate import main as generate
...
from closd.diffusion_planner.data_loaders.humanml.scripts.motion_process import get_target_location, sample_goal, get_allowed_joint_options
...
class TrainLoop:
        ...
        if args.dataset in ['kit', 'humanml'] and args.eval_during_training:
            ...
            self.eval_data = {
                'test': lambda: eval_humanml.get_mdm_loader( self.args,
                    ...
                    args.eval_num_samples, scale=args.gen_guidance_param, hml_type=args.hml_type,
                )
            }
        self.use_ddp = False
    ...
    def generate_during_training(self):
        ...
        if gen_args.multi_target_cond:
            ...
            gen_args.recon_step_start = -1
            gen_args.recon_step_stop = 2
            gen_args.recon_param = 1e3
            gen_args.recon_frame_start = 0
            gen_args.recon_frame_stop = -1
        ...
        if gen_args.multi_target_cond:
            gen_args.target_joint_source = 'random'
            gen_args.autoregressive = False
            allowed_joint_options = get_allowed_joint_options(self.args.target_joint_names)
            allowed_joint_names = [e[:1] for e in allowed_joint_options]
            for interesting_joint_case in [['traj'], ['left_wrist'], ['right_foot']]:
                try:
                    option_idx = allowed_joint_names.index(interesting_joint_case)
                    gen_args.target_joint_names = ','.join(allowed_joint_options[option_idx])
                    gen_args.output_dir = os.path.join(self.save_dir, f'{self.ckpt_file_name()}_{interesting_joint_case[0]}.samples')
                    all_sample_save_path = generate(gen_args)
